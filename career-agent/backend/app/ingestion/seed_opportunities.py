...
async def seed_from_excel(session, filepath: str, source: str):
    ...
    for _, row in df.iterrows():
        ...
        if (await session.execute(select(Opportunity).where(Opportunity.source_id == sid))).first():
            skipped += 1
            continue
        level = str(row.get("Level", "")).strip()
        remote_val = str(row.get("Remote?", "")).strip()
        compensation = str(row.get("Compensation", "")).strip()
        location = str(row.get("Location", "")).strip()
        email = str(row.get("Application Email", "")).strip()
        tags = ",".join(x for x in [
            level if level != "nan" else "",
            f"email:{email}" if email and email != "nan" else "",
        ] if x) or None
        session.add(Opportunity(
            ...
            is_remote=remote_val.lower() == "yes",
            stipend=compensation if compensation != "nan" else None,
...
async def seed_from_curated_csv(session, filepath: str, source: str):
    print(f"Loading {os.path.basename(filepath)}...")
    inserted = skipped = 0
...
async def seed_from_google_sheet(session, sheet_url: str, source: str):
    print(f"Fetching Google Sheet...")
    async with httpx.AsyncClient(timeout=15, follow_redirects=True) as client:
        resp = await client.get(sheet_url)
        if resp.status_code != 200:
            print(f"SKIP: could not fetch sheet (status {resp.status_code})")
            return
    ...
    for row in reader:
        ...
        session.add(Opportunity(
            title=title, organization=org,
            url=url_val or "", source=source,
            is_remote=True, source_id=sid,
        ))
        inserted += 1
    await session.commit()
    print(f"[{source}] inserted={inserted} skipped={skipped}")
...
async def main():
    async with AsyncSessionLocal() as session:
        ...
        curated_files = [
            "deepseek_csv_20260112_4e4575.txt",
            "deepseek_csv_20260112_565406.txt",
            "deepseek_csv_20260112_fefcd2.txt",
        ]
        ...
        for fname in files:
            ...
            if fname.endswith(".xlsx"):
                ...
                await seed_from_excel(session, fpath, source="tech_jobs")
